					...
					<view class="list-item" v-for="(item, index) in list" :key="index" hover-class="hover"
						...
						<view class="content-box">
							<view class="des-box">
								...
								<view v-for="(it,i) in item.cardDataList" @click="copy(it)" class="tit">{{$t('order_number')}}{{i+1}}:{{it}}
								</view>
								<view class="source" :style="{ color:'#4caf50'}">
									<view>
										{{item.goodsInfo}}
									</view>
									<!-- <view>
								...
								<view class="source">
									<view>
										用户等级：<span style="color: red;">{{item.userVipLevel}}</span>
									</view>
									<!-- <view>
										{{$t('reward')}}{{item.bonus/100}}{{$t('yuan')}}
									</view> -->
...
<script>
	...
	export default {
		...
		methods: {
			async getRecord() {
				this.$Request
					.post('/business/getExchangeOrderInfo', {
