	...
	<view class="page">
		...
		<view class="flexView">
			<view class="scrollView">
				<view class="total">总代理收入金额:{{totalAgencyRevenue}}</view>
				<view class="list-view" v-if="!isEmpty">
					<view class="list-item" v-for="(item, index) in list" :key="index" hover-class="hover"
						...
						<view class="content-box">
							<view class="des-box">
								<view class="time">充值用户手机号：{{item.payUserMobile }}</view>
								<view class="tit">提成金额：{{item.amount}}</view>
								<view class="tit">充值金额：{{item.payAmount}}</view>
								<view class="source">
									<view>
										代理等级：<span style="color: red;">{{item.agencyLevel}}</span>
									</view>
								</view>
							</view>
						</view>
...
<script>
	...
	export default {
		...
		methods: {
			async getRecord() {
				this.$Request
					.post('/business/getAgencyRevenueInfo', {
					...
					.then(res => {
						...
						if (res.code == 200) {
							...
							this.totalAgencyRevenue = res.data.totalAgencyRevenue
						} else {
